...
int map6( SDL_Renderer* renderer, int BULLET_DAMAGE, int SKILL_DAMAGE, int CHARGE_RATE ) {
	...
	Player player = { {0,220 - charimg,charimg,charimg} , {0,0}, {0,false,false,true,false} };
	//SDL_Rect player_pos = { 0,0,40,40 };
	double posX = 0, posY = 160-charimg, velX = 0, velY = 0;
	...
	bullet1.vel.x = bulletSpeed*2;
	bullet1.vel.y = 0;

	Bullet bullet3;
	bullet3.pos.x = SCREEN_WIDTH - bulletSize;
	bullet3.pos.y = 60;
	bullet3.pos.w = bulletSize * 2;
	bullet3.pos.h = bulletSize * 2;
	bullet3.vel.x = -bulletSpeed;
	bullet3.vel.y = 0;
    ...
    while( !quit )
    {
		...
		for (int i = 0; i < BULLET_NUMBER_DEFAULT; i++) {
			if (hitMark(bookC, bullets[i])) {
				if (bookC.HP >= BULLET_DAMAGE) {
					...
					else if (player.status.skill_gage + CHARGE_RATE > 100)
		...
		if (isPlayerHitByBullet(player, bullet3)) {
			//printf("You're Dead!\n");
			player.status.game_over = true;
		}
		...
		if (skillBuff)
			SDL_RenderFillRect(renderer, &skill_bullet->pos);
		SDL_RenderFillRect(renderer, &bullet3.pos);
		if (skillBuff)
			SDL_RenderFillRect(renderer, &skill_bullet->pos);
		...
		//*******etc*******
		//ex bullet
		if (bullet1.pos.x > SCREEN_WIDTH) {
			bullet1.pos.x = 0;
			bullet1.pos.y = 200 + rtnRandomNumber(40, 0);
		}
		if (bullet3.pos.x < 0) {
			bullet3.pos.x = SCREEN_WIDTH;
			bullet3.pos.y += 20;
			if (bullet3.pos.y > 420)
				bullet3.pos.y = 60;
		}
		if (bullet2.pos.x < 0) {
